...
/* THE OFFER IS FREE, so there is no PRICE, PRICE_LABEL, CURRENCY_SYMBOL or
   CURRENCY_CODE any more. Two strings do the job all four used to:

     FREE_LABEL  the word that sits where a price used to — the hero card, the
                 docked bars, the trailing accent on a CTA
     CTA_NOTE    the reassurance line under every CTA, replacing the old
                 money-back guarantee

   Both come from the config rather than being written here, so the whole site
...
export const START_DATE = OFFER_CONFIG.startDate;
export const SESSION_TIMES = OFFER_CONFIG.sessionTimes;
export const SESSIONS_LABEL = OFFER_CONFIG.sessionsLabel;
export const SESSION_TIMES_TZ = OFFER_CONFIG.sessionTimesWithZone;

export const WHATSAPP_COMMUNITY_URL = OFFER_CONFIG.whatsappCommunityUrl;
export const CONTACT_EMAIL = OFFER_CONFIG.contactEmail;

/** The three legal pages, in the order they appear in every footer. */
export const LEGAL_LINKS = [
  { href: OFFER_CONFIG.privacyPath, label: 'Privacy Policy' },
  { href: OFFER_CONFIG.termsPath, label: 'Terms of Use' },
  { href: OFFER_CONFIG.refundsPath, label: 'Cancellations' },
];
