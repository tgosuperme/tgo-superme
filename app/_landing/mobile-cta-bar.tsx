...
/**
 * The docked CTA for the two pages AFTER the landing page — registration and
 * confirmation.
 *
 * ./sticky-cta.tsx is the landing page's bar and stays as it is: it is a link
 * to /register with the diary facts on it, and it is always shown. These two
 * pages need something different, so this is a separate component rather than
 * more props on that one:
 *
 *  1. The action is not a link. On /register it advances or submits the form;
 *     on the confirmation page it opens WhatsApp. So the action is passed in
 *     as children and this component only owns the bar.
 *
 *  2. It steps aside. Both pages have their own real CTA in the flow — the
 *     form's own button, the WhatsApp buttons — and a fixed bar sitting on top
 *     of the very button it duplicates reads as a bug. `watch` takes a
 *     selector; while any matching element is on screen the bar slides out and
 *     gives the page back.
 *
 * Mobile and tablet only (`lg:hidden`). Above that the registration summary is
 * a sticky desktop column and the confirmation page fits without help.
 *
...
export default function MobileCtaBar({
  ...
  label: string;
  /** The one fact that sits after it in accent — "Free", normally. */
  trailing?: string;
