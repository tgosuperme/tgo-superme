...
export default function WhatsappWall() {
  ...
  return (
    <div className="mt-16 sm:mt-20">
      ...
      <ul className="mx-auto mt-9 flex max-w-[1160px] flex-wrap items-start justify-center gap-4 sm:gap-5">
        {SHOTS.map((shot, i) => (
          <li
            key={shot.src}
            data-lego=""
            ...
            className="h-[300px] sm:h-[380px] lg:h-[420px] xl:h-[500px]"
          ...
          >
            ...
            >
              {/* SIZED, NOT `fill`, and pointed at the pre-cut card file.
                  ...
                  With `fill` the browser resolved `sizes` against a container
                  whose width came from an inline aspect-ratio it had not
                  computed yet, fell back to the viewport, and asked for the
                  3840px variant — an upscale of an 840px source, which the WASM
                  optimiser then ground away on while the card sat blank.

                  Real width and height plus a file that is already the right
                  size removes both halves of that. The container carries the
                  same ratio as the image, so object-cover crops nothing. */}
              <Image
                src={shot.card}
                alt={shot.alt}
                width={shot.width}
              ...
              {/* Expand, bottom right. It replaces a dark gradient and a "Read
                  it" label that together covered the last quarter of every
                  card — the affordance was sitting on the content it was
                  advertising. */}
              <span
                aria-hidden
              ...
              {/* NOT aria-hidden. A reader using a screen reader has to be told
                  this is an example too, and the alt text alone is easy to miss
                  mid-list. */}
            </button>
          </li>
        ))}
