...
const START = OFFER_CONFIG.startDate;

/* Description stays inside the same compliance line as the page: no outcome
   promise, no percentage, no "pain-free". The price it used to end on is now
   the word free, which is also the strongest thing in the sentence, so it
   keeps that final position. */
const DESCRIPTION = `A live, coach-led 5-day pain reset challenge for adults 35+ with persistent back, neck or knee pain. Guided movement, breath work, strengthening and real-time correction. Starts ${START}, live on Zoom, completely free.`;

const TITLE = 'Free 5-Day Pain Reset Challenge | SuperMe';
    ...
    <html lang="en-GB" className={inter.variable}>
      <body>
        ...
        {/* One pair of observers for the whole document, mounted here rather
            ...
            page, the registration form and the thank-you page. Renders
            nothing. */}
        <LegoObserver />
        {/* Sets the _fbp cookie and captures ?fbclid into _fbc, both of which
            the registration form POSTs to our server so /api/register can send
        ...
        {/* Last-touch UTM + first-touch referrer/landing_url into localStorage
            and a first-party cookie, read again when the form is submitted.
            Every attribution field on the CRM row comes from here. */}
        <AttributionCapture />
        <GoogleAnalytics />
        <Clarity />
        {children}
