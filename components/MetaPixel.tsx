 ...
 * PageView is the ONLY event this Pixel sends. atc_event, ic_event and
 * registration_complete are server-side only, via the Conversions API. There
 * is deliberately no `trackCustom` helper here any more: exporting one is an
 * open invitation to reintroduce a browser half, and a browser half that is
 * not deduplicated against the server copy double-counts every conversion.
 *
...
/**
 * Reads a cookie in the browser. Used for `_fbp` and `_fbc`, which are sent to
 * our own server with each tracked action: a CAPI event is assembled
 * server-side and cannot read the reader's cookies for itself.
 */
 ...
 * finished loading by the time the reader submits, and the one where an ad
