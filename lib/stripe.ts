...
/**
 * Stripe client, used by the VIP upgrade ONLY.
 *
 * ── SCOPE, BECAUSE THIS FILE ONCE COVERED THE WHOLE FUNNEL ──────────────────
 * Registering for the challenge is free and touches none of this. Stripe is
 * reached exactly once, when someone chooses VIP on the OTO page, and a
 * failure anywhere in here must never be able to stop a free registration. The
 * routes are separated for that reason: /api/register knows nothing about
 * Stripe, and /api/checkout is only ever called from /upgrade.
 *
...
export function getStripe(): Stripe {
  ...
  if (!client) {
    client = new Stripe(key, {
      appInfo: { name: 'SuperMe 5-Day Pain Reset (VIP)', version: '2.0.0' },
    });
  }
  return client;
}
